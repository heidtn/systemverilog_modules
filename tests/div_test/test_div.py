...
def print_status(dut):
    print(f"quotient {dut.quotient.value.signed_integer} accum {dut.accum.value.signed_integer}")
    print(f"nextquotient {dut.quotient_next.value.signed_integer} nextaccum {dut.accum_next.value.signed_integer}")
    print(f"denom {dut.denom.value.signed_integer}")
    print("raw")
    print(f"quotient {dut.quotient.value} accum {dut.accum.value}")
    print(f"nextquotient {dut.quotient_next.value} nextaccum {dut.accum_next.value}")
    print("")
...
async def test_two_nums(a, b, dut):
    ...
    if((a < 0) != (b < 0)):
        expected *= -1
    if DEBUG:
        print(f"Result quotient {dut.quotient_next} and remainder {dut.accum_next}") 
        print(f"Got: {dut.o_result.value.signed_integer} Should be {expected} for {a}/{b}")
        print("")
...
async def test_div(dut):
    ...
    nums1 = [5, 8.5, 4.0, 1.241412, -3, -0.5, 22, 4, 0]
    nums2 = [7, 3.3, -2.5, 1.5, 4.4, 3, 1, 3.3, -1]
